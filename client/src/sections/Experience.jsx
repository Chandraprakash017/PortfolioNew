            ...
            <div className="grid gap-4 grid-cols-1 sm:grid-cols-2">
              {certificates.map((cert, idx) => (
                <motion.div
                  ...
                  viewport={{ once: true }}
                  transition={{ duration: 0.4, delay: idx * 0.1 }}
                  className="glass p-4 rounded-2xl flex flex-col justify-center border-t-2 border-t-primary-400/50"
                >
                  <h4 className="font-bold text-sm text-light-text dark:text-dark-text mb-2 line-clamp-2">{cert.title}</h4>
                  <span className="text-xs font-semibold text-primary-600 dark:text-primary-400 uppercase tracking-widest">{cert.date}</span>
                </motion.div>
              ))}
            </div>
          </div>
